...
what I want to do:
1- save numpy array for 3 seconds the data order should never change. Like the first values should be from myoITU etc. I mean use device names. Okay we can do that. Just make sure.

2- currently we are saving a numpy arrray add that numpy array a coulmn at its begining which will store the gesture label which retrieved from user but since we cannot write a string inside our numpy array the gestrue name retrieved from user would be store in dictionary (probobaly) with its corresponding name. 

3- These numpy arrays will be saved into dataframe inside model train function.
Why a data frame because the model without data frame gives low accuracy levels since it treats calibration for the same gesture as different time files. This ml algorithm implementation accuracy is low in order to solve this we tried better feature extraction , windowing etc. But while data files are only 300 rows there is only a few windows.
Also we want data frame to later save this information as csv
